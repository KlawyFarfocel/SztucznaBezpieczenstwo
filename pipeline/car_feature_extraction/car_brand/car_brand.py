...
async def detect_car_brand_async(frame, model):
    loop = asyncio.get_running_loop()

    def sync_task():
        device = "0" if USE_GPU() else "cpu"
        if USE_GPU():
            results = model(frame, device=device, half=True, conf=0.6, verbose=False)
        else:
            results = model(frame, device=device, conf=0.6, verbose=False)
